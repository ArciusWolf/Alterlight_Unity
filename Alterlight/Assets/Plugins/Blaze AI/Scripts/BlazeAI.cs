...
public class BlazeAI : MonoBehaviour 
{
    ...
    void OnAnimatorMove()
    {
        if (!useRootMotion || navmeshAgent.isOnOffMeshLink) {
            return;
        }
    ...
    void NonRootMotionMovement(string anim, float animT, float moveSpeed, float turnSpeed, string dir)
    {
    ...
    void RootMotionMovement(string anim, float animT, float turnSpeed)
    {
        RotateTo(pathCorner, turnSpeed);
        navmeshAgent.isStopped = true;
        animManager.Play(anim, animT);
    ...
    {
        if (!waypoints.useMovementTurning) return true;
    ...
    void OffMeshLinkJump(float speed)
    {
        if (!useOffMeshLinks) {
            return;
        }
    ...
    bool SearchForLadder()
    {
        ...
        int numColliders = Physics.OverlapSphereNonAlloc(transform.position + vision.visionPosition, navmeshAgent.radius + 0.3f, hitColliders, ladderLayers);
        ...
        for (int i=0; i<numColliders; i++) 
        {
            ...
            chosenLadder = hitColliders[i].transform;
            return true;
        }
    ...
    void VisionCheck()
    {
        ...
        for (int i=0; i<visionCollNum; i++) 
        {
            ...
            if (enemyToAttack == null || vision.checkMaxSightOnTargetLock && enemyToAttack != null) {
        ...
        enemyColPoint = enemiesToAttack[0].ClosestPoint(transform.position + vision.visionPosition);
    ...
    GameObject CheckSurroundingForTarget()
    {
        ...
        int maxColliders = 13;
        Collider[] hitColliders = new Collider[maxColliders];
        int numColliders = Physics.OverlapSphereNonAlloc(transform.position + vision.visionPosition, enemyContactRadius, hitColliders, vision.hostileAndAlertLayers);
    ...
    void RunEnemyOnEnterEvent()
    {
        ...
        enemyEnterEvent.Invoke();
    }

    void RunEnemyOnLeaveEvent()
    {
        if (enemyToAttack == null) return;
        enemyLeaveEvent.Invoke();
    }
    ...
    public void SetState(State stateToTurnTo, bool forceSetInOffMesh = false)
    {
    ...
    public void Death(bool callOthers = false, GameObject enemy = null, bool comingFromDeathDoll = false)
    {
    ...
    public void DeathDoll(float timeToRagdoll, bool callOthers = false, GameObject enemy = null)
    {
        // return if already dead or Blaze disabled
        if (state == State.death || !enabled) {
    ...
    public void RemoveDistanceCulling(bool enableObject=false)
    {
        ...
        PrintWarning(warnAnomaly, "Operation skipped: can't remove agent from distance culling list since no distance culling instance can be found.");
    }
