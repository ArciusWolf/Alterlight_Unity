...
namespace BlazeAISpace
{
    ...
    public class CoverShooterBehaviour : MonoBehaviour
    {
        ...
        bool CanSeeTarget(bool onAttacking = false)
        {
            ...
            int hits = Physics.SphereCastNonAlloc(transform.position + blaze.vision.visionPosition, 0.05f, (targetDir - blaze.vision.visionPosition).normalized, hitResults, blaze.distanceToEnemy + 5f, layers);
        ...
        {   
            ...
            List<RaycastHit> hitList = new List<RaycastHit>();
            int hits = Physics.SphereCastNonAlloc(transform.position + blaze.vision.visionPosition, 0.15f, transformDir, hitResults, (blaze.navmeshAgent.radius * 2) + blaze.navmeshAgent.height/2, layersToHit);
        ...
        public void CallOthers()
        {
            ...
            for (int i=0; i<callOthersNum; i+=1) {
                ...
                if (!callPassesColliders) 
                {
                    ...
                    Transform target = callOthersColl[i].transform;
                    Collider currentColl = callOthersColl[i];
        ...
        public void EquipWeapon()
        {
            ...
            blaze.RotateTo(blaze.enemyToAttack.transform.position, turnSpeed);
            EquipTimer();
        }
